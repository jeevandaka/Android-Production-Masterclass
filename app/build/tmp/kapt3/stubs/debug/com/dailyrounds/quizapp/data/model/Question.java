...
/**
 * Domain/UI model for a single quiz question.
 *
 * Kept as one flat model (no separate DTO) because the JSON shape returned by the
 * assignment's gist maps 1:1 onto what the UI needs. If a real backend later adds
 * fields we don't want, split this into QuestionDto (data/remote) + Question
 * (domain) and map between them in the repository -- the seam is already
 * isolated there, so it's a small, low-risk change.
 */
